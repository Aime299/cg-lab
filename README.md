好的，我来补全运行方法部分。以下是完整的 README，包含了三个实验的详细运行方法：

```powershell
code README.md
```

**复制以下完整内容：**

```markdown
# CG-Lab 计算机图形学课程作业

## 学生信息

- **姓名**：付雅婷
- **班级**：24级计算机科学与技术（公费师范）
- **课程**：计算机图形学
- **教师**：张鸿文
- **助教**：张怡冉

---

## 实验一：图形学开发工具

### 环境配置

#### 开发工具

- **IDE**：Trae (VS Code 内核)
- **包管理器**：uv (Rust 编写的高性能 Python 包管理器)
- **版本控制**：Git

#### 依赖安装

```bash
uv add taichi
```

### 项目结构（src 布局）

```
CG-Lab/
├── src/
│   ├── Work0/                    # 实验一：粒子系统
│   │   ├── __init__.py
│   │   ├── config.py             # 配置参数
│   │   ├── physics.py            # 物理计算
│   │   └── main.py               # 主程序入口
│   ├── Work1/                    # 实验二：三角形旋转
│   │   ├── __init__.py
│   │   ├── main.py               # 主程序入口
│   │   └── transform.py          # 变换矩阵实现
│   └── Work2/                    # 实验三：3D立方体旋转（选做）
│       ├── __init__.py
│       ├── main.py               # Pygame版本主程序
│       └── transform.py          # NumPy变换矩阵
├── .gitignore
├── pyproject.toml
├── README.md
├── work0.gif                     # 实验一演示动图
├── work1.gif                     # 实验二演示动图
└── work2.gif                     # 实验三演示动图
```

### 运行方法

#### 实验一：粒子系统

```bash
# 进入项目目录
cd D:\桌面\CG-Lab

# 同步依赖
uv sync

# 运行程序
uv run -m src.Work0.main
```

**运行说明**：
- 程序会弹出 GUI 窗口，显示粒子群运动
- 移动鼠标可影响粒子运动轨迹
- 按 ESC 键退出程序

#### 实验二：三角形旋转

```bash
# 进入项目目录
cd D:\桌面\CG-Lab

# 运行程序
uv run -m src.Work1.main
```

**运行说明**：
- 程序会弹出 700x700 窗口，显示彩色三角形
- 按 **A** 键：三角形顺时针旋转
- 按 **D** 键：三角形逆时针旋转
- 按 **ESC** 键：退出程序

#### 实验三：3D立方体旋转（选做）

```bash
# 进入项目目录
cd D:\桌面\CG-Lab

# 安装依赖（首次运行）
uv pip install pygame numpy

# 运行程序
python src/Work2/main.py
```

**运行说明**：
- 程序会弹出 800x600 窗口，显示 3D 立方体线框
- **A/D**：绕 Y 轴旋转
- **W/S**：绕 X 轴旋转
- **Q/E**：绕 Z 轴旋转
- **Space**：暂停/继续自动旋转
- **R**：重置所有角度
- **ESC**：退出程序

---

## 实验一：万有引力粒子群仿真

### GPU 加速情况 ✅

程序成功调用 **NVIDIA CUDA** 进行 GPU 并行计算：

```
[Taichi] version 1.7.4, llvm 15.0.1, commit b4b956fd, win, python 3.12.13
[Taichi] Starting on arch=cuda
```

> **说明**：这是最理想的 GPU 调用状态，粒子群模拟性能最优。

### 运行效果演示

![实验一演示](work0.gif)
https://work0.gif

### 功能说明

- **万有引力模拟**：粒子之间根据距离和质量产生引力
- **GPU 并行计算**：使用 Taichi 将计算部署到 GPU
- **实时交互**：鼠标移动产生力场影响粒子运动
- **平滑渲染**：60fps 实时可视化

### 代码分层设计

- **config.py**：集中管理配置参数（粒子数量、引力常数、窗口大小等）
- **physics.py**：实现万有引力计算和粒子位置更新（GPU 并行）
- **main.py**：主程序入口，负责 GUI 初始化、渲染循环和鼠标交互

---

## 实验二：旋转与变换（三角形）

### 实验目标

深入理解 3D 空间中的坐标变换流程（MVP 变换），独立推导并用代码实现模型变换、视图变换和投影变换矩阵。

### 三角形顶点坐标

- v0: (2.0, 0.0, -2.0)
- v1: (0.0, 2.0, -2.0)
- v2: (-2.0, 0.0, -2.0)

### 核心功能

#### 1. 模型变换 (Model Transformation)

绕 Z 轴旋转的变换矩阵

#### 2. 视图变换 (View Transformation)

将相机平移到世界坐标系原点

#### 3. 投影变换 (Projection Transformation)

透视投影矩阵，分为两步：

- 透视平截头体挤压为正交长方体
- 正交投影至规范立方体 [-1, 1]³

### 交互操作

- 按 **A** 键：三角形顺时针旋转
- 按 **D** 键：三角形逆时针旋转
- 按 **ESC** 键：退出程序

### 运行效果演示

![实验二演示](work1.gif)
https://work1.gif


---

## 实验三：3D 立方体透视旋转（选做）

### 实现方式

- 使用 **Pygame** 进行渲染
- 使用 **NumPy** 进行矩阵运算
- 实现了完整的 MVP 变换管线

### 立方体信息

- 8 个顶点，12 条边
- 边长 2，中心在原点
- 顶点坐标范围 [-1, 1]

### 功能特性

- **多轴旋转**：A/D(绕Y)、W/S(绕X)、Q/E(绕Z)
- **自动旋转**：按 Space 切换自动/手动模式
- **重置功能**：按 R 重置所有角度
- **透视投影**：60°视场角，0.1-100 裁剪平面
- **相机控制**：相机位于 (0,0,5)，看向原点

### 交互操作

| 按键 | 功能 |
|------|------|
| A/D | 绕 Y 轴旋转 |
| W/S | 绕 X 轴旋转 |
| Q/E | 绕 Z 轴旋转 |
| Space | 暂停/继续自动旋转 |
| R | 重置所有角度 |
| ESC | 退出程序 |

### 运行效果演示

![实验三演示](work2.gif)
https://work2.gif

---

## 实验任务完成情况

### 实验一

- [x] 基础图形学开发环境搭建（uv + Taichi）
- [x] src 布局重构和代码分层（config, physics, main）
- [x] GPU 粒子系统实现（万有引力仿真）
- [x] Git 代码托管（GitHub + README + 演示动图）

### 实验二

- [x] 模型变换矩阵（绕 Z 轴旋转）
- [x] 视图变换矩阵（相机平移）
- [x] 透视投影矩阵
- [x] 三角形线框渲染
- [x] A/D 键控制旋转
- [x] 代码提交到 GitHub
- [x] 演示动图上传

### 实验三（选做）

- [x] 3D 立方体构建（8顶点+12边）
- [x] 绕 X/Y/Z 轴多角度旋转矩阵
- [x] 透视投影和视图变换
- [x] Pygame 渲染实现
- [x] 交互控制（A/D/W/S/Q/E/R/Space）
- [x] 自动旋转模式
- [x] 代码提交到 GitHub
- [x] 演示动图上传

---

## 技术栈

| 工具/库 | 用途 | 版本 |
|---------|------|------|
| Python | 编程语言 | 3.12.13 |
| Taichi | GPU 并行计算图形学库 | 1.7.4 |
| Pygame | 3D 渲染框架 | 最新 |
| NumPy | 矩阵运算 | 最新 |
| uv | 包管理器 | 最新 |
| Git | 版本控制 | 2.54.0 |

---

## 参考资料

- [Taichi 官方文档](https://docs.taichi-lang.org/)
- [uv 包管理器文档](https://docs.astral.sh/uv/)
- [实验课程主页](https://zhanghongwen.cn/cg)
- [src 布局说明](https://packaging.pythonlang.cn/en/latest/discussions/src-layout-vs-flat-layout/)

## 仓库地址

[https://github.com/Aime299/cg-lab](https://github.com/Aime299/cg-lab)
